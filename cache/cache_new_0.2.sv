...
module cache (
    ...
    (* ram_style = "block" *)reg [32*(1 << OFFSET_WIDTH)-1:0] cache_memory_w0 [CACHE_DEPTH-1:0]; 
    (* ram_style = "block" *)reg [TAG_WIDTH-1:0] cache_tag_w0 [CACHE_DEPTH-1:0];
    
    logic valid_bits_w0 [CACHE_DEPTH-1:0];
    logic dirty_bits_w0 [CACHE_DEPTH-1:0];
            ...
            case (state)
                IDLE: begin
                    if (readtrigger || writetrigger) begin
                        ...
                        cache_tag_reg <= cache_tag_w0[input_addr[13:4]];
                        cache_data_reg <= cache_memory_w0[input_addr[13:4]];
                        
                        dirty_bit_reg <= dirty_bits_w0[input_addr[13:4]];
                        valid_bit_reg <= valid_bits_w0[input_addr[13:4]];
                        state <= COMP;
                    end
                end
                ...
                COMP: begin
                    if (read_req_reg) begin
                        if (valid_bit_reg && (cache_tag_reg == tag_reg)) begin // HIT
                         ...
                         if (valid_bit_reg && (cache_tag_reg == tag_reg)) begin // HIT
                            case (offset_reg)
                                2'b00: cache_memory_w0[index_reg] <= {cache_data_reg[127:32], input_data_reg};
                                2'b01: cache_memory_w0[index_reg] <= {cache_data_reg[127:64], input_data_reg, cache_data_reg[31:0]};
                                2'b10: cache_memory_w0[index_reg] <= {cache_data_reg[127:96], input_data_reg, cache_data_reg[63:0]};
                                2'b11: cache_memory_w0[index_reg] <= {input_data_reg, cache_data_reg[95:0]};
                            endcase
