...
module fdiv (
    ...
    reg [47:0] lut [0:4095]; // 1.23形式�?�初期近似値を�?�納するLUT
    initial begin
    ...
    wire [23:0] diff_24 = diff[46:23]; // 上�?24ビットを�?り�?��?
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            st1_sign <= s_a ^ s_b;
            st1_exponent <= {2'b0, calc_e_a} - {2'b0, calc_e_b} + 10'd127; // 初期�?数の計�?
            ...
            st1_mantissa_a <= m_a;
            st1_mantissa_b <= m_b;
            st1_x0 <= lut[m_b[22:13]]; // m_bの上�?8ビットをイン�?�?クスにして初期近似値を取�?
            st1_y0_dy <= lut[m_b[22:11]]; // m_bの上�?8ビットをイン�?�?クスにして初期近似値を取�?
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            st2_valid <= st1_valid; // 単純にパイプラインを�?�める
            st2_sign <= st1_sign;
            st2_exponent <= st1_exponent; // �?数は初期段階で計算済み

            st2_nan <= st1_nan;
            st2_inf <= st1_inf;
            st2_zero <= st1_zero;

            st2_mantissa <= st1_mantissa_a; // 初期近似値を使って割り算�?�近似値を計�?
            st2_mantissa_b <= st1_mantissa_b; // 後�?�段階で使用するため保持
