...
module fdiv (
    ...
    reg [23:0] st2_mantissa, st2_mantissa_b, st2_x1;

    // ================================================================
    // Stage 1: Initial parsing & LUT lookup
    // ================================================================
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            st1_valid <= 1'b0;
            st1_sign <= 1'b0; st1_exponent <= 10'b0;
            ...
            st1_sign <= s_a ^ s_b;
            st1_exponent <= {2'b0, calc_e_a} - {2'b0, calc_e_b} + 10'd127;
            
            // Bが2のべき乗か（Fractionがオールゼロか）を判定してフラグ化
    ...
    // ================================================================
    // Stage 2: Taylor Expansion (DSP48 Inference P = C - A * B)
    // ================================================================
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            st2_valid <= 1'b0;
            st2_sign <= 1'b0; st2_exponent <= 10'b0;
            st2_nan <= 1'b0; st2_inf <= 1'b0; st2_zero <= 1'b0;
            st2_b_is_power2 <= 1'b0;
            st2_mantissa <= 24'b0; st2_mantissa_b <= 24'b0; st2_x1 <= 24'b0;
        end else begin
            st2_valid <= st1_valid;
            st2_sign <= st1_sign; st2_exponent <= st1_exponent;
            st2_nan <= st1_nan; st2_inf <= st1_inf; st2_zero <= st1_zero;
            st2_b_is_power2 <= st1_b_is_power2;
            st2_mantissa <= st1_mantissa_a;
            st2_mantissa_b <= st1_mantissa_b;
            st2_x1 <= x1_taylor;
        end
    end

    // ================================================================
    // Stage 3: Final Multiplication & Rounding
    // ================================================================
    // Bが2のべき乗なら誤差ゼロなのでバイアス無し。それ以外はテイラーの打ち切り誤差を相殺
    wire [47:0] bias = st2_b_is_power2 ? 48'h0 : 48'h0000_007F_FFFF;
    wire [47:0] q_biased = (st2_mantissa * st2_x1) + bias;

    wire norm_shift = q_biased[46];
    
    wire [23:0] raw_mantissa = norm_shift ? q_biased[46:23] : q_biased[45:22];
    wire round_bit           = norm_shift ? q_biased[22] : q_biased[21];
    
    wire [24:0] rounded_mantissa = {1'b0, raw_mantissa} + {24'b0, round_bit};
    
    // 指数部の補正（符号なし変数が混ざらないようにすべて signed）
    wire signed [10:0] shift_adj = norm_shift ? 11'sd0 : -11'sd1;
    wire signed [10:0] round_adj = rounded_mantissa[24] ? 11'sd1 : 11'sd0;
    wire signed [10:0] final_exp = st2_exponent + shift_adj + round_adj;
        ...
        if (!rst_n) begin
            ...
            if (st2_nan) begin
                ...
                if (st2_exponent > 10'sd255 || (st2_exponent == 10'sd255 && st2_mantissa >= st2_mantissa_b)) begin
                    result <= {st2_sign, 8'hFF, 23'b0};
                end else if (final_exp >= 11'sd255) begin
                    result <= {st2_sign, 8'hFF, 23'b0};
                end else if (final_exp <= 11'sd0) begin
                    result <= {st2_sign, 31'b0};
                end else begin
                    result <= {st2_sign, final_exp[7:0], rounded_mantissa[22:0]};
                end
